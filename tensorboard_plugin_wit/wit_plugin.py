...
class WhatIfToolPlugin(base_plugin.TBPlugin):
    ...
    def __init__(self, context):
        ...
        self._wit_data_dir = (
            context.flags.wit_data_dir if context.flags else None
        )
        ...
        if context.flags and context.flags.custom_predict_fn:
            try:
                ...
                spec = iu.spec_from_file_location(
                    "custom_predict_fn", context.flags.custom_predict_fn
                )
                ...
                logger.error(
                    "Have you defined a function named custom_predict_fn?"
                )
    ...
    def frontend_metadata(self):
        return base_plugin.FrontendMetadata(
            es_module_path="/index.js", tab_name="What-If Tool"
        )
    ...
    def _serve_wit(self, request):
        ...
        filepath = os.path.join(
            os.path.dirname(__file__), "static", "wit_tb_bin.html"
        )
        with io.open(filepath, encoding="utf-8") as infile:
            contents = infile.read()
    ...
    def _serve_wit_js(self, request):
        del request  # unused
        filepath = os.path.join(
            os.path.dirname(__file__), "static", "wit_tb_bin.js"
        )
        with io.open(filepath, encoding="utf-8") as infile:
            contents = infile.read()
        return werkzeug.Response(
    ...
    def generate_sprite(self, example_strings):
        ...
        feature_list = (
            self.examples[0].features.feature
        ...
        self.sprite = (
            ...
            if (len(self.examples) and self.image_feature_name in feature_list)
            else None
        )
    ...
    def _examples_from_path_handler(self, request):
        ...
        """
        start_example = (
            int(request.args.get("start_example"))
            if request.args.get("start_example")
            else 0
        )
        if not start_example:
            ...
            try:
                ...
                return http_util.Respond(
                    request, e.message, "application/json", code=400
                )
            except Exception as e:
                return http_util.Respond(
                    request, str(e), "application/json", code=400
                )
        ...
        return http_util.Respond(
            request,
            {
                ...
                "sprite": True
                if (self.sprite and not start_example)
                else False,
                "next": end_example,
            },
            "application/json",
    ...
    def _update_example(self, request):
        ...
        if index >= len(self.examples):
            return http_util.Respond(
                request, "invalid index provided", "application/json", code=400
            )
    ...
    def _infer(self, request):
        ...
        if not start_example:
            ...
            try:
                ...
                logging.error(str(e))
                return http_util.Respond(
                    request, e.details, "application/json", code=400
                )
            except Exception as e:
                logging.error(str(e))
                return http_util.Respond(
        ...
        def get_inferences_resp():
            ...
            if request.args.get("model_type") == "classification":
                for obj in sliced_infer_objs:
                    ...
                    ]["classifications"][start_example:end_example]
            else:
                for obj in sliced_infer_objs:
    ...
    def _infer_mutants_handler(self, request):
        ...
        try:
            ...
            json_mapping = self._infer_mutants_impl(
                feature_name,
                example_index,
                inference_addresses,
                model_names,
